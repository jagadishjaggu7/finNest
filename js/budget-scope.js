    ...
    async function render() {
        ...
        async function paint() {
            ...
            try {
                ...
                host.innerHTML = `<div class="budget-context"><span>${scope === 'family' ? '👨‍👩‍👧 Shared household budget' : '👤 Your personal budget'}</span><strong>${scope === 'monthly' ? '' : ''}${info.start.toLocaleDateString('en-IN',{day:'numeric',month:'short'})}${period === 'monthly' ? ` · ${info.start.toLocaleDateString('en-IN',{month:'long',year:'numeric'})}` : ` – ${info.end.toLocaleDateString('en-IN',{day:'numeric',month:'short'})}`} · ${money(totalSpent)} spent / ${money(totalBudget)} budgeted</strong></div>
                    <div class="budget-editor-row"><select id="scopeCategory">${window.FinNestBudgetService.CATEGORIES.map(category => `<option>${escapeHtml(category)}</option>`).join('')}</select><input id="scopeAmount" type="number" min="0" step="100" placeholder="₹ limit"><button class="finnest-primary-button" id="scopeAdd">Set Budget</button></div>
                    <div class="budget-scope-grid">${data.budgets.length ? data.budgets.map(budget => { const used = Number(spent[budget.category] || 0); const limit = Number(budget.amount || 0); const pct = limit ? Math.min(100, used / limit * 100) : 0; const over = used > limit; return `<div class="budget-scope-row"><div><div class="budget-scope-name">${escapeHtml(budget.category)}</div><div class="budget-scope-meta">${money(used)} spent · ${over ? 'Over budget' : money(Math.max(0, limit-used)) + ' remaining'}</div></div><input type="number" min="0" step="100" value="${limit}" data-budget-id="${budget.id}"><div class="budget-scope-actions"><button data-save-id="${budget.id}">Save</button><button class="danger" data-delete-id="${budget.id}">Delete</button></div><div style="grid-column:1/-1;height:7px;background:#E2E8F0;border-radius:99px;overflow:hidden"><div style="height:100%;width:${pct}%;background:${over?'#EF4444':'#10B981'};border-radius:99px"></div></div></div>`; }).join('') : '<div class="budget-scope-empty">No budgets yet for this scope and period. Use the controls above to create one.</div>'}</div>`;
                ...
                host.querySelectorAll('[data-save-id]').forEach(button => button.onclick = async () => {
                    ...
                    try {
                        await window.FinNestBudgetService.save({ scope, period, category: data.budgets.find(b => String(b.id) === String(button.dataset.saveId))?.category, amount: Number(input.value), id: button.dataset.saveId });
                        window.dispatchEvent(new CustomEvent('finnest:budget-changed', { detail: { scope, period } }));
                        await paint();

...
(function () {
    ...
    function styles() {
        ...
        style.textContent = `
            ...
            .cloud-family-row{display:grid;grid-template-columns:38px minmax(0,1fr) auto;gap:10px;align-items:center;padding:10px;border:1px solid #E2E8F0;border-radius:12px;background:#fff}
            .cloud-family-avatar{width:34px;height:34px;border-radius:50%;display:grid;place-items:center;background:#ECFDF5;color:#047857;font-size:11px;font-weight:800}
            .cloud-family-main{min-width:0}.cloud-family-role{display:block;margin-top:3px;color:#94A3B8;font-size:10px}
            .cloud-family-row input{width:100%;box-sizing:border-box;padding:9px 10px;border:1px solid #CBD5E1;border-radius:9px;font:inherit;color:#334155}
            .cloud-family-row input:focus{outline:none;border-color:#10B981;box-shadow:0 0 0 3px rgba(16,185,129,.10)}
            .cloud-family-save{border:1px solid #D1FAE5;background:#fff;color:#047857;border-radius:9px;padding:9px 11px;font-weight:700;cursor:pointer}
            .cloud-family-save:disabled{opacity:.55;cursor:wait}
            .cloud-family-empty{padding:16px;border:1px dashed #CBD5E1;border-radius:12px;color:#64748B;font-size:12px}
        `;
    ...
    async function saveMember(member, value, button, user) {
        ...
        try {
            ...
            if (typeof familyMembers !== 'undefined') {
                familyMembers = familyMembers.map((name, index) => index === 0 && member.user_id === user.id ? value : name);
            }
        ...
        try {
            ...
            if (!context.members.length) {
                ...
                host.querySelectorAll('[data-save-member]').forEach(button => button.onclick = async () => {
                    ...
                    await saveMember(member, value, button, context.user);
                    input.value = value;

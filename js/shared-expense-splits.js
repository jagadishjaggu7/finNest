...
(function () {
    ...
    async function syncLatestSplit(){
        ...
        try{
            ...
            const {data:expenses}=await supabase.from('expenses').select('id,household_id,amount,expense_type,updated_at').eq('user_id',user.id).eq('expense_type','shared').order('updated_at',{ascending:false}).limit(1);
            const expense=expenses?.[0];if(!expense?.household_id)return;
            const ids=[...selected].filter(id=>members.some(m=>m.id===id));if(!ids.length)return;
            ...
            await supabase.from('expenses').update({paid_by_member_id:payerId}).eq('id',expense.id);
            await supabase.from('expense_splits').delete().eq('expense_id',expense.id);await supabase.from('expense_splits').insert(rows);
        }finally{syncing=false;}
    }

    function hookTypeButtons(){document.querySelectorAll('.type-option').forEach(btn=>{if(btn.dataset.splitHooked)return;btn.dataset.splitHooked='1';btn.addEventListener('click',()=>{lastExpenseType=btn.dataset.type||'personal';updateVisibility();if(lastExpenseType==='shared')loadMembers().then(renderField);});});}

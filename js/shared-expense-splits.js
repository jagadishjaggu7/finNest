...
(function () {
    ...
    function addStyles() {
        ...
        style.textContent = `
            #expensePayerField{display:none;margin:12px 0;padding:12px;border:1px solid #D1FAE5;background:#F0FDF4;border-radius:14px}
            #expensePayerField.shared-visible{display:block}
            #expensePayerField label{display:block;margin:0 0 7px;font-size:12px;font-weight:700;color:#334155}
            #expensePayer{width:100%;min-height:44px;padding:10px 12px;border:1px solid #CBD5E1;border-radius:10px;background:#fff;color:#334155;font:inherit;cursor:pointer}
            .shared-members{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px;margin-bottom:12px}
            .shared-member{display:flex;align-items:center;gap:7px;padding:9px;border:1px solid #E2E8F0;border-radius:10px;background:#fff;font-size:12px;cursor:pointer}
    ...
    function renderField() {
        ...
        field.querySelectorAll('.shared-member input').forEach(input => input.addEventListener('change', e => {
            if (e.target.checked) selectedMemberIds.add(e.target.value); else selectedMemberIds.delete(e.target.value);
        }));
        field.querySelector('#expensePayer')?.addEventListener('change', e => { payerId = e.target.value || null; });
    }
    ...
    function currentType() { return document.querySelector('.type-option.selected')?.dataset.type || 'personal'; }

    async function onTypeClick(event) {
        const option = event.currentTarget;
        const type = option.dataset.type;
        if (type !== 'shared') { showForType('personal'); return; }
        try { await loadMembers(); } catch (error) { console.warn('FinNest could not load family members', error); members = []; }
        showForType('shared');
    }
    ...
    function bindSave() {
        ...
        button.addEventListener('click', () => {
            ...
            // Let app.js perform its normal save first; then attach the member/payer data to that newest cloud expense.
    ...
    async function syncNewestSharedExpense() {
        ...
        } catch (error) { console.warn('FinNest shared payer sync failed', error); }
    }
    ...
    async function init() {
        ...
        try { await loadMembers(); renderField(); } catch (error) { console.warn('FinNest family members unavailable', error); renderField(); }
        document.addEventListener('finnest:authenticated', async () => { try { await loadMembers(); renderField(); if (currentType() === 'shared') showForType('shared'); } catch (error) { console.warn('FinNest family refresh failed', error); } });
    }

    window.FinNestSharedExpense = { reload: async () => { await loadMembers(); renderField(); }, showForType, getPayerId: () => payerId };

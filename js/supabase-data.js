...
(function () {
    ...
    async function syncAll() {
        ...
        try {
            const currentUser = await user(); if (!currentUser) return; const accounts = await accountsFor(currentUser.id); const map = idMap();
            ...
            for (const expense of expenses || []) { const uuid = map.expenses[expense.id]; const row = { user_id: currentUser.id, household_id: null, paid_by_member_id: null, amount: Number(expense.amount), category: expense.category || "Other", account_id: accounts[expense.account] || null, expense_type: expense.type === "shared" ? "shared" : "personal", note: expense.note || null, expense_date: expense.date || todayString() }; const result = uuid ? await client().from("expenses").update(row).eq("id", uuid).eq("user_id", currentUser.id).select("id").maybeSingle() : await client().from("expenses").insert(row).select("id").single(); if (result.error) throw result.error; if (result.data?.id) { map.expenses[expense.id] = result.data.id; wantedExpenseIds.add(result.data.id); } }
